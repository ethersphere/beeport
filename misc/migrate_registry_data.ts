...
async function fetchBatchEvents() {
  try {
    ...
    // Fetch all BatchCreated events
    const events = await provider.getLogs({
      address: CONTRACT_ADDRESS,
      fromBlock: START_BLOCK,
      toBlock: currentBlock,
      topics: [
        batchCreatedEvent.topicHash,
      ],
    });
    ...
    // Process events
    const batchData = await Promise.all(events.map(async (event) => {
    ...
    const validBatchData = batchData.filter((data): data is NonNullable<typeof data> => data !== null);

    // Group batches by owner
    const batchesByOwner: { [owner: string]: typeof validBatchData } = {};
