...
module.exports = __toCommonJS(stamper_exports);
...
function signMessage(message, privateKey) {
  const signature = import_secp256k1.secp256k1.sign(keccak256(message), privateKey, { prehash: false });
...
function recoverPublicKey(message, r, s, v) {
  ...
  const signature = new import_secp256k1.secp256k1.Signature(r, s, recovery);
...
function privateKeyToPublicKey(privateKey) {
  ...
  const point = import_secp256k12.secp256k1.Point.BASE.multiply(privateKey);
  return [point.x, point.y];
}
function compressPublicKey(publicKey) {
  return import_secp256k12.secp256k1.Point.fromAffine({ x: publicKey[0], y: publicKey[1] }).toBytes(true);
}
function publicKeyFromCompressed(compressed) {
  ...
  const point = import_secp256k12.secp256k1.Point.fromBytes(compressed);
  return [point.x, point.y];
}
...
var ETHEREUM_SIGNED_MESSAGE_PREFIX = ENCODER2.encode("Ethereum Signed Message:\n32");
function personalSignDigest(data) {
  const bytes = data instanceof Uint8Array ? data : ENCODER2.encode(data);
