...
function privateKeyToPublicKey(privateKey) {
  ...
  const point = secp256k12.Point.BASE.multiply(privateKey);
  return [point.x, point.y];
}
function compressPublicKey(publicKey) {
  return secp256k12.Point.fromAffine({ x: publicKey[0], y: publicKey[1] }).toBytes(true);
}
function publicKeyFromCompressed(compressed) {
  ...
  const point = secp256k12.Point.fromBytes(compressed);
  return [point.x, point.y];
}
