...
export default function DepthCalc() {
  ...
  const [details, setDetails] = useState<{ [key: string]: any } | null>(null);

  const depthToVolume: { [key: number]: string } = {
    22: "4.93 GB",
    23: "17.03 GB",
    24: "44.21 GB",
    25: "102.78 GB",
  ...
  const validateInputs = (
    ...
    if (!Number.isInteger(depthValue) || depthValue < 17 || depthValue > 41) {
      localErrors.depth =
        "Depth must be an integer greater or equal to 17 and less than or equal to 41.";
    }
    ...
    if (Object.keys(localErrors).length === 0) {
      ...
      setResult(
        `A depth value of ${cleanedDepth} allows for an effective storage volume of ${effectiveVolume} and a theoretical max volume of ${theoreticalMaxVolume}. For an amount value of ${amountValue} PLUR / block / chunk it can be stored for ${formattedTime} at a cost of ${costInxBZZ.toFixed(
          2
        )} xBZZ.`
      );
      setDetails({
    ...
    >
      <div style={{ marginBottom: "10px" }}>
        ...
        <label
          ...
          style={{ display: "block", marginBottom: "5px" }}
        >
          Amount (current minimum is {loading ? "Loading..." : price * 17280}{" "}
          PLUR):
        </label>
        <input
          id="amountInput"
