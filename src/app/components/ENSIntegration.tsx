  ...
  const handleDomainRegistration = async () => {
    ...
    try {
      if (registrationStep === 'input') {
        ...
        await publicClient.waitForTransactionReceipt({ hash: commitHash, pollingInterval: 8_000 });
...
This waiting period is required by ENS to prevent front-running attacks where someone could see your registration attempt and register the domain before you.`);
        ...
        // Wait for transaction confirmation
        await publicClient.waitForTransactionReceipt({ hash: registerHash, pollingInterval: 8_000 });
...
`);
  ...
  const handleSetContentHash = async () => {
    ...
    try {
      ...
      const receipt = await publicClient.waitForTransactionReceipt({ hash, pollingInterval: 8_000 });
