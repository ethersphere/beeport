...
export interface ClientSideUploadResult {
  ...
  /**
   * Set when the post-upload "save issuer state to SOC" step ran. Absent if
   * the SOC save failed or was skipped — the file upload itself still
   * succeeded. See {@link saveIssuerStateToSOC}.
   */
  issuerStateSoc?: {
...
export async function uploadFileClientSide(
  ...
  // The file's root chunk is itself a chunk that must be uploaded — finalize()
  // does NOT push it through onChunk.
  ...
  //    bucket counters. The SOC payload captures state PRE-save; the local
  //    state below is the authoritative POST-save one. See IssuerStateSOC.ts
  //    for the drift discussion.
  let issuerStateSoc: ClientSideUploadResult['issuerStateSoc'];
  try {
    ...
    // SOC failures must NEVER fail the upload — the user's file is already on
    // Swarm. Log it so issuer-state recovery is debuggable but otherwise
    // continue.
...
async function maybeSaveIssuerStateToSOC(
  ...
  hotKey: DerivedHotKey,
  onStatus?: (msg: string) => void
): Promise<ClientSideUploadResult['issuerStateSoc']> {
  try {
    onStatus?.('Saving issuer state to Swarm (SOC)…');
    const soc = await saveIssuerStateToSOC({
      bee: ctx.bee,
...
export interface MultiFileUploadResult {
  ...
  totalChunks: number;
  issuerStateSoc?: ClientSideUploadResult['issuerStateSoc'];
}
